...
/*
   ...
   File Description  :  Implementation of the date/time parsing logic.
...
# if (_MSC_VER > 1914) && (BOOST_VERSION >= 106700)
#  pragma warning(disable:5032)
# endif // #if (_MSC_VER > 1914) && (BOOST_VERSION >= 106700)
# pragma warning(push)
# if _MSC_VER < 1400
...
# if (_MSC_VER > 1914) && (BOOST_VERSION >= 106700)
#  pragma warning(disable:4643 5026 5027 5031 5032)
# endif // #if (_MSC_VER > 1914) && (BOOST_VERSION >= 106700)
	...
	int secs_to_add)
{
	if (secs_to_add) {
		...
		if ((tmp_double <
			...
			throw std::invalid_argument("The requested number of seconds to add (" +
...
{
	if (in_date == NULL)
		throw std::invalid_argument("Specified date string pointer is 'NULL'.");
	...
	try {
		...
		if (date_length == 8) {
			...
			if (end_ptr != (in_date + 8))
				throw std::invalid_argument("Invalid undelimited date string --- "
			...
			if ((!isdigit(in_date[0])) || (!isdigit(in_date[1])) ||
				...
				throw std::invalid_argument("Invalid delimited date string --- "
			...
			if (date_length == 10) {
				...
				if (((in_date[10] != ' ') && (in_date[10] != '.')) ||
					...
					throw std::invalid_argument("Invalid delimited date/time string.");
				if (date_length == 13)
				...
				throw std::invalid_argument("Date/time string length exceeds maximum "
